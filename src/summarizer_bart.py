...
class Summarizer:
    ...
    def summarize(cls, text):
        ...
        input_ids = cls.tokenizer.encode(text, return_tensors="pt", max_length=1024, truncation=True).to('cuda')
        summary_ids = cls.model.generate(input_ids,
                                    length_penalty=2.0, num_beams=4,
                                    early_stopping=True)
        output_summ = [cls.tokenizer.decode(g, skip_special_tokens=True,
